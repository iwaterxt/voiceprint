...
SplitRadixComplexFft::SplitRadixComplexFft(MatrixIndexT N) {
  if ( (N & (N-1)) != 0 || N <= 1)
    std::cerr << "SplitRadixComplexFft called with invalid number of points "
              << N;
  N_ = N;
  logn_ = 0;
  while (N > 1) {
...
SplitRadixComplexFft::SplitRadixComplexFft(
    const SplitRadixComplexFft &other):
  ...
  memcpy(brseed_, other.brseed_, sizeof(MatrixIndexT) * brseed_size);
  ...
  if (logn_ < 4) {
    ...
    for (MatrixIndexT i = logn_; i >= 4 ; i--) {
      ...
      tab_[i-4] = new BaseFloat[this_array_size];
      memcpy(tab_[i-4], other.tab_[i-4],
                  sizeof(BaseFloat) * this_array_size);
    }
  }
}
...
void SplitRadixComplexFft::BitReversePermute(BaseFloat *x, MatrixIndexT logn) const {
...
void SplitRadixComplexFft::ComputeRecursive(BaseFloat *xr, BaseFloat *xi, MatrixIndexT logn) const {
  ...
  /* Check range of logn */
  if (logn < 0)
    std::cerr << "Error: logn is out of bounds in SRFFT";
